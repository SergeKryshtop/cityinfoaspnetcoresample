...
#Clone the kube-deploy repo, and run worker.sh on the worker machine with root:
yum install -y git

git clone https://github.com/kubernetes/kube-deploy
cd kube-deploy/docker-multinode

export MASTER_IP=$K8S_MASTER
export USE_CNI=True
